...
| H-021 | readDicom returns metadata but unsupported transfer syntax silently substitutes wrong pixel data | Phase 2.2 ships decoders for the 11 GDCM-bundled transfer syntaxes. If a syntax outside that whitelist (HTJ2K, MPEG family, big-endian, Deflated, JPIP) were read and the implementation defaulted to "decode as raw" rather than refusing, viewer code would render garbage as the patient image — false positives or missed findings. | S5 | P3 | U | Single source of truth: `cpp/dicom_read.cpp::kSupportedSyntaxes`. `readDicomFile` whitelist-checks the file's transfer-syntax UID before calling `gdcm::ImageReader`; on mismatch sets `image.hasPixelData = false` and `pixelDataBase64 = null`. The TS surface mirrors the whitelist via `isSupportedTransferSyntax()` (SR-0013) so callers can pre-filter studies. `writeSyntheticDicom` rejects out-of-whitelist UIDs at write time, preventing tests/fixtures from accidentally producing files we can't safely read. Whitelist is expanded explicitly per decoder added in future phases (HTJ2K when libopenjp2 is updated; MPEG never planned). | SR-0011, SR-0013 | S5 | P1 | L | Mitigated (Phase 2.2) |
...
| 2026-04-29 | Vivek Sah (self) | Phase 2.2 boundary | H-021 residual risk lowered from M → L: mitigation now includes (i) whitelist check on read, (ii) whitelist mirror exposed to JS via `isSupportedTransferSyntax()`, (iii) write-side rejection for unsupported UIDs, (iv) automated Jest test asserting the negative path, (v) on-device PASS/FAIL grid in the example app covering all 11 supported syntaxes plus an unsupported negative control (MPEG2MainProfile). Status moved Identified → Mitigated. No new hazards opened in Phase 2.2. |
